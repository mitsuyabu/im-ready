-- ============================================================
-- update_plan_blueprint_section を replace して、許可セクションに 'accommodations' を追加する。
--
-- 20260905 / 20260906 / 20260908 と同じ設計（security invoker / owner check / search_path 固定 /
-- for update ロック / jsonb_set で 1 セクションだけ patch / 他セクション保持 /
-- p_expected_updated_at 任意の optimistic concurrency）。変更点は許可セクションのみ。
--
-- accommodations は My Plan の「滞在方法」（ホームステイ / シェアハウス / 学生寮 等）を
-- 期間つきで持つ配列。plan_karte（shared understanding）は書き換えない。値の形は array。
-- ============================================================
...
begin
  -- 許可セクションのみ（data の既存キー名に一致）。accommodations を追加。
  if p_section not in (
    'goals', 'destinations', 'workInterests', 'thingsToDo', 'milestones', 'schools',
    'planSettings', 'accommodations'
  ) then
    raise exception 'invalid_section: %', p_section;
  end if;

  -- 値の型ガード。destinations / planSettings は object、それ以外（accommodations 含む）は array。
