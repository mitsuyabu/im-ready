...
import {
  buildWorksheetLetterSystemPrompt,
  type WorksheetLetterConcreteDetails,
} from "@/lib/worksheetLetterPrompt";
import type {
...
// 優先順位カテゴリと同じ発想の id 検証。/api/worksheet-summary・/api/worksheet-note の
// 実装とは独立させ、あちらのコードには触れない（小さな重複を許容する。3ファイル目）。
...
/**
 * ワークシートの回答から、親に見せられる資料（手紙形式）を生成する単発エンドポイント。
 * 既存の /api/worksheet-note・/api/worksheet-summary・優先順位カテゴリの入力・チャット・
 * 提案・カルテ・localStorage保存には一切依存しない、独立した機能。結果はその場表示のみで保存しない。
 * concreteDetails（希望国・都市・学校名・確定予算等）は将来の拡張の受け口で、今回は常に null。
 */
export async function POST(req: NextRequest) {
  ...
  // 将来、希望国・都市・学校名・確定予算等が渡せるようになるまでは常に null
  const concreteDetails: WorksheetLetterConcreteDetails = null;

  try {
    const response = await anthropic.messages.create({
      model: MODEL,
      max_tokens: 2048,
      system: buildWorksheetLetterSystemPrompt({
        freeTextByCategory,
        priorities,
        nextStep,
        concreteDetails,
      ...
      messages: [
        {
          role: "user",
          content: "上記のデータを踏まえ、親に見せる資料（手紙形式）を書いてください。",
        },
      ],
    });
    ...
    const letter = textBlock ? stripMarkdownBold(textBlock.text.trim()) : "";
    ...
    console.error("worksheet letter error:", isApiError ? err.message : err);
