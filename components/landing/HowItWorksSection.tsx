 ...
 * 各stepは 番号（accent色、Mobile text-2xl〜Desktop text-4xl程度でtitleに次ぐ2番目の
 * 存在感）→title（大きく黒、Chat/Worksheet/My Planを直接示す短い語句）→lead（中間サイズ、
 * 黒、既存の文言を流用）→description（body、黒85%）という4段階の階層。優先順位は
 * title＞番号＞lead/descriptionの順。text/visual列幅（lg:w-4/12 / lg:w-8/12）・container
 * 幅（max-w-6xl）・section gap・Imageのsizes（50vw）は既存のまま変更していない。
